...
Capa 7: **Renombramiento políglota con conocimiento de la estructura del árbol sintáctico (AST).** El comando `forkctl rename plan` genera una diferencia (diff) que se puede revisar, que abarca archivos de identidad, símbolos de código (26 lenguajes a través de ast-grep) y superficies textuales no relacionadas con el código. El comando `forkctl rename apply` crea una instantánea del árbol, ejecuta todas las fases, regenera los archivos de bloqueo y deja un manifiesto de regeneración de activos para cualquier elemento binario. El comando `forkctl rename rollback` restaura la última instantánea. No utiliza cadenas de comandos `sed`. Corrige los límites de las palabras. Considera las mayúsculas y minúsculas.
...
Forkctl se encarga de "todo lo demás".
...
Forkctl se ofrece tanto como un **servidor MCP** (transporte a través de stdio, para clientes MCP como Claude Code) como una **interfaz de línea de comandos (CLI)** con la misma funcionalidad.
...
### Evaluación
- `forkctl_assess`: puntuación de preparación para la adopción, bloqueos, fortalezas.
- `forkctl_choose_path`: bifurcar | plantilla | importar | clonar (sin seguimiento).
- `forkctl_make_forkable`: corrige el repositorio original (por defecto: plan; opcional: PR).

### Ejecución
- `forkctl_preflight_policy`: detecta bloqueos relacionados con las políticas de bifurcación de la empresa/organización/repositorio.
- `forkctl_create_fork`: crea una bifurcación de forma asíncrona, devuelve un ID de operación.
- `forkctl_create_from_template`: utiliza la función `/generate` de GitHub.
- `forkctl_check_operation`: verifica el estado de cualquier operación en curso.

### Configuración inicial
- `forkctl_bootstrap`: configuración inicial basada en perfiles (contribuidor / kit de inicio / repositorio semilla interno / entrega al cliente / experimento).
- `forkctl_configure_upstream`: configura el repositorio remoto y, opcionalmente, el flujo de sincronización.
- `forkctl_scan_drift`: busca rutas codificadas, secretos filtrados y referencias obsoletas del sistema de integración continua (CI) en la copia.
- `forkctl_emit_handoff`: genera un único artefacto con información precisa: URLs, comandos, advertencias y la siguiente acción a realizar.

### Sincronización
- `forkctl_sync`: API de GitHub para fusionar los cambios con el repositorio original.
- `forkctl_diagnose_divergence`: muestra los commits pendientes, los archivos en riesgo y los posibles conflictos.
- `forkctl_propose_sync_pr`: propone una solicitud de extracción (PR) para la sincronización cuando la fusión rápida falla.

### Gestión
- `forkctl_list_forks`: lista tus bifurcaciones y las que estás siguiendo, con una columna de estado.
- `forkctl_fleet_health`: verifica el estado de las bifurcaciones (obsoletas, con conflictos o abandonadas).
- `forkctl_batch_sync`: sincroniza por lotes, teniendo en cuenta los límites de velocidad.

### Registros
- `forkctl_receipt`: registro legible por máquina de cualquier operación.
- `forkctl_audit_log`: historial de solo escritura.

### Renombrar (Capa 7: novedad en la versión 1.1.0)
- `forkctl_rename_plan`: planificador de renombramiento con conocimiento de la estructura del árbol sintáctico; genera una diferencia que se puede revisar.
- `forkctl_rename_apply`: crea instantáneas y aplica cambios a archivos de identidad, símbolos, texto y fases posteriores.
- `forkctl_rename_rollback`: restaura a partir de la última instantánea.
...
| `FORKCTL_STATE_DIR` | no       | Directorio de estado del usuario del sistema operativo (a través de `env-paths`). | Ubicación de las operaciones de SQLite y la base de datos de auditoría. |
...
- Forkctl nunca realiza "push" forzados, elimina repositorios ni elimina ramas.
