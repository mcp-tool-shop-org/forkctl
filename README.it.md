...
Livello 7 – **Rinomina polivalente consapevole dell'AST**. Il comando `forkctl rename plan` genera una diff revisionabile che interessa i file di identità, i simboli del codice (26 linguaggi tramite ast-grep) e le superfici testuali non relative al codice. Il comando `forkctl rename apply` crea uno snapshot dell'albero, esegue tutte le fasi, rigenera i file di blocco e lascia un file di manifest per la rigenerazione degli asset binari. Il comando `forkctl rename rollback` ripristina l'ultimo snapshot. Nessuna catena di comandi `sed`. Corretto per i confini delle parole. Sensibile alle maiuscole/minuscole.
...
Forkctl si occupa di "tutto il resto".
...
Forkctl è disponibile sia come **server MCP** (trasporto tramite standard input/output, per Claude Code e altri client MCP) sia come **interfaccia a riga di comando (CLI)** con la stessa funzionalità.
...
## I ventidue strumenti
<!-- FORKABLE_COUNTS_END -->

### Valutazione
- `forkctl_assess` — punteggio di prontezza all'adozione, blocchi, punti di forza
- `forkctl_choose_path` — fork | template | import | clone-detached
- `forkctl_make_forkable` — corregge il repository originale (impostazione predefinita: pianificazione; opzionale: pull request)

### Esecuzione
- `forkctl_preflight_policy` — rileva eventuali blocchi relativi alle politiche di fork dell'organizzazione, del repository o dell'azienda
- `forkctl_create_fork` — crea un fork in modo asincrono, restituisce l'ID dell'operazione
- `forkctl_create_from_template` — utilizza l'API GitHub `/generate`
- `forkctl_check_operation` — verifica lo stato di qualsiasi operazione in corso

### Configurazione iniziale
- `forkctl_bootstrap` — configurazione guidata da profili (contributore / kit di avvio / seed interno / consegna al cliente / esperimento)
- `forkctl_configure_upstream` — imposta il repository remoto, configura il flusso di lavoro di sincronizzazione opzionale
- `forkctl_scan_drift` — verifica la presenza di percorsi hardcoded, segreti esposti o riferimenti obsoleti al sistema di integrazione continua nella copia
- `forkctl_emit_handoff` — fornisce un singolo artefatto completo: URL, comandi, avvertenze, prossima azione

### Sincronizzazione
- `forkctl_sync` — API di GitHub per la sincronizzazione con il repository originale
- `forkctl_diagnose_divergence` — verifica la presenza di commit non sincronizzati, file a rischio, potenziali conflitti
- `forkctl_propose_sync_pr` — crea una pull request per la sincronizzazione in caso di fallimento della sincronizzazione diretta

### Gestione
- `forkctl_list_forks` — elenca i tuoi fork e quelli monitorati, con colonna relativa allo stato
- `forkctl_fleet_health` — verifica lo stato di fork obsoleti, con conflitti o abbandonati
- `forkctl_batch_sync` — sincronizzazione in batch, con limiti di velocità

### Registrazioni
- `forkctl_receipt` — record leggibile dalle macchine di qualsiasi operazione
- `forkctl_audit_log` — registro storico delle attività

### Rinomina (Livello 7 – novità della versione 1.1.0)
- `forkctl_rename_plan` – pianificatore di rinomina consapevole dell'AST; genera una diff revisionabile.
- `forkctl_rename_apply` – crea snapshot e applica modifiche a file di identità, simboli, superfici testuali ed esegue passaggi successivi.
- `forkctl_rename_rollback` – ripristina dall'ultimo snapshot.
...
| `FORKCTL_STATE_DIR` | no       | Directory per lo stato dell'utente del sistema operativo (tramite `env-paths`) | Dove risiedono le operazioni SQLite e il database di audit. |
