...
レイヤー7 — **AST（抽象構文木）を認識する多言語対応のリネーム機能**。`forkctl rename plan`は、IDファイル、コードシンボル（ast-grepによる26の言語）、および非コードのテキスト領域全体にわたる、レビュー可能な差分を出力します。`forkctl rename apply`は、ツリーの最新状態を保存し、すべての処理を実行し、ロックファイルを再生成し、バイナリファイルの場合はアセット再生成のマニフェストを残します。`forkctl rename rollback`は、最新の保存状態に戻します。`sed`コマンドの連鎖は使用しません。単語境界を正確に処理し、大文字小文字を考慮します。
...
forkctlは、その「それなりの作業」をすべてカバーします。
...
forkctlは、**MCPサーバー**（標準入出力経由での通信、Claude CodeなどのMCPクライアント用）と、同じ機能を持つ**CLI**として提供されます。
...
### 評価
- `forkctl_assess`：導入の準備状況のスコア、問題点、利点
- `forkctl_choose_path`：フォーク | テンプレート | インポート | 分離クローン
- `forkctl_make_forkable`：ソースリポジトリの修正（デフォルト：計画；オプション：プルリクエスト）

### 実行
- `forkctl_preflight_policy`：エンタープライズ/組織/リポジトリのフォークポリシーに関する問題点の検出
- `forkctl_create_fork`：非同期実行、操作IDを返す
- `forkctl_create_from_template`：GitHubの`/generate`を使用
- `forkctl_check_operation`：実行中の操作の状態を確認

### 初期設定
- `forkctl_bootstrap`：プロファイルに基づいた初期設定（貢献者向け / スターターキット向け / 内部向け / クライアント向け / 実験向け）
- `forkctl_configure_upstream`：リモートの設定、オプションの同期ワークフローの設定
- `forkctl_scan_drift`：ハードコードされたパス、漏洩した秘密情報、古いCIリファレンスの検出
- `forkctl_emit_handoff`：URL、コマンド、注意点、次のアクションを含む、単一の信頼できる成果物

### 同期
- `forkctl_sync`：GitHubのmerge-upstream API
- `forkctl_diagnose_divergence`：未コミットの変更、リスクのあるファイル、予測される競合
- `forkctl_propose_sync_pr`：高速フォワードが失敗した場合のプルリクエストベースの同期

### 一括管理
- `forkctl_list_forks`：自分のフォークと監視中のフォークの一覧表示（ヘルス状態の列付き）
- `forkctl_fleet_health`：古いフォーク、競合のあるフォーク、放棄されたフォーク
- `forkctl_batch_sync`：レート制限を考慮した一括同期

### 記録
- `forkctl_receipt`：すべての操作に関する機械可読な記録
- `forkctl_audit_log`：追記専用の履歴

### リネーム（レイヤー7 — v1.1.0で新機能）
- `forkctl_rename_plan` — ASTを認識するリネームプランナー。レビュー可能な差分を出力します。
- `forkctl_rename_apply` — 最新状態を保存し、ID、シンボル、テキスト、および後処理を適用します。
- `forkctl_rename_rollback` — 最新の保存状態から復元します。
...
| `FORKCTL_STATE_DIR` | no       | OSのユーザー状態ディレクトリ（`env-paths`経由）。 | SQLiteの操作と監査データベースが存在する場所。 |
