...
**Nobody in the fork-rebrand space does AST-aware rename.** That is the gap forkable
...
```bash
forkable rename plan    <path> --from <old> --to <new> [flags]
forkable rename apply   <path> --plan <plan.json> [flags]
forkable rename rollback <path>                      # restore latest snapshot
```
...
- `forkable_rename_plan` — returns a `RenamePlan` (JSON, user-reviewable).
- `forkable_rename_apply` — consumes a `RenamePlan`, returns a `RenameReceipt`.
...
| Variant | Example (`forkable` → `splitshift`) |
|---|---|
...
**Identifier-boundary rule is non-negotiable.** `forkable` must never match inside
...
For repos with heavy TS — and forkable itself is one — ast-grep's identifier match
misses a few real-world cases:

- Scope-aware shadowing (a local `forkable` variable in a function should **not** be
...
- Re-exports and barrel files: `export { Forkable } from './forkable'` needs the
...
3. **Asset regeneration manifest.** Write `.forkable/asset-regen.json` listing every
...
`forkable rename plan` produces a `RenamePlan` JSON artifact with three sections:
...
```json
{
  ...
  "variants": {
    ...
    "snake_case":     { "from": "forkable",  "to": "splitshift",  "enabled": true },
    "camelCase":      { "from": "forkable",  "to": "splitshift",  "enabled": true },
...
```json
{
  "layers": {
    ...
    "post":       { "lockfilesToRegenerate": ["package-lock.json"], "pathsToMove": ["src/forkable/ → src/splitshift/"], "assetsToRegen": ["site/public/favicon.png"] }
  },
  "excluded": ["node_modules/", "dist/", ".git/", "CHANGELOG.md entries before fork point"],
  "warnings": [
    "Found 3 instances of 'forkable' inside string literals that may be incidental mentions — review files/list"
  ]
}
```
...
disk (`.forkable/rename-plan.diff`) so the user can review it in their editor
before `forkable rename apply`.
...
| `"forkable"` in a code-comment | Renamed (textual pass walks comments in markdown; for source-code comments, ast-grep `kind: comment` match applied with variant regex). |
| `"forkable"` in a string literal in source code | **Default: rewritten.** Flagged in warnings. User can suppress specific files via `exclude`. |
| Markdown code fence containing `forkable` | Untouched (code fences are skipped in textual; ast-grep handles anything that's real code). |
...
| Directory `src/forkable/` | `git mv` to `src/splitshift/`, imports updated via symbols pass. |
...
| Re-export `export { Forkable } from './forkable'` | Covered by ast-grep symbols pass (specifier) + identity pass (filename) + path rename (directory). `--deep-ts` catches edge cases. |
...
Before `apply` mutates anything, a snapshot is written to `.forkable/snapshots/rename-<timestamp>/`:
...
`forkable rename rollback` restores the latest snapshot. Snapshots kept for 7 days,
garbage-collected on next `forkable` command.
...
- Tests: all 7 variants for representative inputs including edge cases
  (`forkable` / `forkable-2` / `forkable_v2` / acronyms like `URL` / multi-word)
...
- Wire all passes into `rename-plan` (read-only) and `rename-apply` (snapshot → apply
  → verify)
- `.forkable/rename-plan.json` and `.forkable/rename-plan.diff` artifacts
...
1. **`--deep-ts` as default?** Research brief suggests opt-in. For forkable itself
...
4. **Renaming the git remote.** Out of scope for rename itself; forkable already has
...
- **Semantic rename of concepts.** Renaming the *product* is one string (plus its
  ...
  `Account`) is a different tool and NOT what forkable rename does.
