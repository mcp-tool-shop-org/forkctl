...
forkable has very few knobs by design. Everything that needs to vary per deployment is an env var; everything that's a behavioral choice is a bootstrap profile.
...
| `FORKABLE_STATE_DIR` | no | OS user-state directory (via `env-paths`) | Where the SQLite operations and audit DB live. Override for tests or shared runners. |
...
Override with `FORKABLE_STATE_DIR=/path/to/dir`.

The DB is opened in WAL mode and is safe for concurrent reads. Writes from multiple forkable processes against the same DB are serialized by SQLite.
...
| `workflow` | Required when `forkable_configure_upstream` installs `.github/workflows/sync-upstream.yml` |
| `read:org` | Lets `forkable_preflight_policy` read org-level fork policy (members_can_fork_private_repositories) |
...
```bash
forkable bootstrap my-org/copy --profile contributor --no-apply
```
...
| `make-forkable` | `--branch <name>` | `forkable/adoption-fixes` | Custom branch name for the fix PR |
...
| `propose-sync-pr` | `--sync-branch <name>` | `forkable/sync-from-upstream` | Use a different branch name on the fork |
| `propose-sync-pr` | `--pr-title <title>` | `forkable: sync from upstream` | Custom PR title |
...
```bash
forkable assess octocat/hello-world --json
# → { "ok": true, "data": { "score": 87, "blockers": [...], ... } }

forkable create-fork bad/ref --json
