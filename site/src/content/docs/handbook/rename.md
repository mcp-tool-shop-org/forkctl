...
> New in **v1.1.0**. Layer 7 of forkable. Read-only plan, snapshotted apply, one-command rollback.

When you adopt a repo and decide to call it your own, every reference to the original name needs to change — the `package.json` `name`, the `Cargo.toml` `[package].name`, the exported `Forkable` class, the README H1, the repo URL in workflow files, the `FORKABLE_API_KEY` that's really just the env prefix, the directory called `src/forkable/`. Every one of those, in every casing variant, with word boundaries that don't break `starship` when you rename `star`.

`forkable rename` does that in one command.

## What it does — concretely

Given `--from forkable --to splitshift`, the rename derives the casing variant set:

| Variant | From | To |
|---|---|---|
| kebab-case | `forkable` | `splitshift` |
| snake_case | `forkable` | `splitshift` |
| camelCase | `forkable` | `splitshift` |
...
```bash
# 1. Plan (read-only). Emits .forkable/rename-plan.{json,diff}.
forkable rename plan . --from forkable --to splitshift

# 2. Review the diff in your editor.
$EDITOR .forkable/rename-plan.diff

# 3. Apply. Snapshots the tree first.
forkable rename apply . --plan .forkable/rename-plan.json

# 4. If anything looks wrong, roll back.
forkable rename rollback .
```
...
2. **Path renames.** If `src/forkable/` exists, `git mv` to `src/splitshift/`. Handles case-insensitive filesystems (Windows, macOS default) with a two-step `.tmp` rename.
3. **Asset regeneration manifest.** Writes `.forkable/asset-regen.json` listing every binary asset that likely needs to be remade. Never modified; surfaced as a TODO list.
...
Plan artifacts live at `.forkable/rename-plan.json` and `.forkable/rename-plan.diff`. Snapshots at `.forkable/snapshots/rename-<timestamp>/`. All three paths are under the repo root, so `.gitignore` them if you don't want snapshots tracked.

## Exclusions (always)

forkable never touches `.git/`, `node_modules/`, `bower_components/`, `dist/`, `build/`, `out/`, `.next/`, `.nuxt/`, `.astro/`, `target/`, `__pycache__/`, `.venv/`, `venv/`, `env/`, `coverage/`, minified JS/CSS, sourcemaps, or binary files by extension (`.png`, `.jpg`, `.ico`, `.woff*`, `.ttf`, `.otf`, `.wasm`, `.so`, `.dll`, `.dylib`, `.exe`, `.bin`, `.mp3`, `.mp4`, `.webm`, `.pdf`, `.zip`, `.tar*`, `.gz`).
...
Snapshots live for 7 days, garbage-collected on the next `forkable` command.

## Troubleshooting

Every failure is a structured `ForkableError` with a code. See [Troubleshooting — Rename](../troubleshooting/#rename-new-in-v110) for the full per-code recovery reference. Quick pointers:

- Apply failed mid-run → `forkable rename rollback <path>` then read `details.layer`.
