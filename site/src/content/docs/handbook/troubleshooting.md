...
description: Error code reference, common scenarios, and where to look when forkable misbehaves.
sidebar:
  order: 7
---
...
If you read something here that's wrong, that's a bug — [open an issue](https://github.com/mcp-tool-shop-org/forkable/issues) with the template at the bottom of this page.

## Error code reference

Every forkable tool returns either `{ ok: true, data }` or `{ ok: false, error: { code, message, hint?, details? } }`. The `code` is one of the values below. The `message` is human copy. The `hint` — when present — is the fastest next step.
...
**What to check.** Did you export it in the shell that launched forkable? For MCP, is it in the `env` block of the server config?
...
For MCP, put it in the server config `env`, not in a separate dotfile — forkable-mcp inherits from the spawning process.
...
**What to check.**
- Are you pointing at the right state dir? Set `FORKABLE_STATE_DIR` if you use a non-default location.
...
**How to recover.** Look at `details.filesAtRisk` and `details.predictedConflicts`. If the fork has genuine local commits you want to keep, go the PR route. If not, you can reset locally and force-pull — but forkable won't do that for you.

#### `SYNC_BRANCH_EXISTS`

**What it means.** `propose_sync_pr` tried to create a branch (default: `forkable/sync-from-upstream`) but that branch already exists on the fork.
...
```bash
forkable propose-sync-pr myhandle/my-fork --branch forkable/sync-from-upstream-retry
```
...
**What it means.** forkable threw an exception we didn't classify. Not your fault.
...
```bash
forkable preflight-policy source-owner/source-repo
```
...
3. **Use `clone-detached`** — forkable doesn't need a GitHub fork relationship to manage a repo. You lose upstream sync, but you gain a repo that actually exists.
4. **Ask `choose-path`** which path is best given your goal:

```bash
forkable choose-path source-owner/source-repo --goal internal_seed
```
...
forkable deliberately does *not* try to auto-resolve conflicts. That's a human call. Your options:

1. **Resolve in the PR UI.** GitHub's conflict editor handles most cases.
2. **Pull the branch locally,** resolve in your editor, push.
3. **Abandon this sync.** Close the PR. You can always re-sync later — forkable keeps no state that locks you in.
...
Usually means `batch-sync` across many forks, or you're running forkable alongside another GitHub-heavy tool on the same token.
...
- Long-term: give forkable its own token distinct from your dev tooling.
...
```bash
forkable bootstrap myhandle/my-new-fork \
...
- The `forkable_emit_handoff` tool gives you a truthful snapshot of what bootstrap actually did vs. skipped.

## Where to look

### The state DB

forkable keeps its operations DB and audit log in a single SQLite file:

- **Default path.** OS user-state dir, resolved via [`env-paths`](https://www.npmjs.com/package/env-paths) with app name `forkable`:
  - Windows: `%LOCALAPPDATA%\forkable\Data\forkable-state.db`
  - macOS: `~/Library/Application Support/forkable/forkable-state.db`
  - Linux: `~/.local/share/forkable/forkable-state.db`
- **Override.** Set `FORKABLE_STATE_DIR` to any directory.
...
```bash
sqlite3 "$(forkable doctor --state-path 2>/dev/null || echo ~/.local/share/forkable/forkable-state.db)" \
...
If the state DB is wedged (corrupt schema, stuck operation you can't clear, an upgrade warning you can't get past), reset is safe — forkable is designed to re-derive truth from GitHub:

1. Back up the existing file if you want the audit history.
2. Delete it.
3. Re-run any tool — forkable will recreate the DB on first touch.

```bash
# Linux example
mv ~/.local/share/forkable/forkable-state.db ~/forkable-state.db.bak
forkable assess octocat/hello-world   # will recreate
```
...
- forkable crashes without a structured error (stack trace makes it to the user).
- A documented scenario doesn't match what forkable actually does.
...
```markdown
**forkable version:** (output of `forkable --version`)
...
Issue tracker: https://github.com/mcp-tool-shop-org/forkable/issues
