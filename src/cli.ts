...
function makeContext(toolName?: string): ToolContext {
  ...
  const renameOnly = toolName !== undefined && toolName.startsWith("forkctl_rename_");
...
program
  ...
  .action(async (repo, opts) => {
    ...
    await run("forkctl_assess", input, opts);
  });
...
program
  ...
  .option("--json", "raw JSON output")
  .action(async (repo, opts) => {
    await run("forkctl_choose_path", { repo, goal: opts.goal }, opts);
  });
...
program
  ...
  .option("-b, --branch <branch>", "branch name for pr mode", "forkctl/adoption-fixes")
  .option("--json", "raw JSON output")
  .action(async (repo, opts) => {
    await run(
...
program
  ...
  .option("--json", "raw JSON output")
  .action(async (repo, opts) => {
    await run("forkctl_preflight_policy", { repo }, opts);
  });
...
program
  ...
  .action(async (source, opts) => {
    ...
    await run("forkctl_create_fork", input, opts);
  });
...
program
  ...
  .action(async (template, opts) => {
    ...
    await run("forkctl_create_from_template", input, opts);
  });
...
program
  ...
  .action(async (operationId, opts) => {
    await run("forkctl_check_operation", { operationId }, opts);
  });
...
program
  ...
  .action(async (destination, opts) => {
    ...
    await run("forkctl_bootstrap", input, opts);
  });
...
program
  ...
  .action(async (destination, opts) => {
    ...
    if (opts.source) input.source = opts.source;
    await run("forkctl_scan_drift", input, opts);
  });
...
program
  ...
  .action(async (destination, opts) => {
    ...
    await run("forkctl_emit_handoff", input, opts);
  });
...
program
  ...
  .action(async (fork, opts) => {
    ...
    await run("forkctl_sync", input, opts);
  });
...
program
  ...
  .action(async (fork, opts) => {
    const input: Record<string, unknown> = { fork };
    if (opts.branch) input.branch = opts.branch;
    await run("forkctl_diagnose_divergence", input, opts);
  });
...
program
  ...
  .option("-s, --sync-branch <name>", "name for the sync branch", "forkctl/sync-from-upstream")
  .option("-t, --pr-title <title>", "PR title", "forkctl: sync from upstream")
  .option("--json", "raw JSON output")
  .action(async (fork, opts) => {
    const input: Record<string, unknown> = {
      fork,
    ...
    if (opts.branch) input.branch = opts.branch;
    await run("forkctl_propose_sync_pr", input, opts);
  });
...
program
  ...
  .action(async (opts) => {
    ...
    if (opts.source) input.source = opts.source;
    await run("forkctl_list_forks", input, opts);
  });
...
program
  ...
  .option("--json", "raw JSON output")
  .action(async (opts) => {
    await run("forkctl_fleet_health", { limit: opts.limit }, opts);
  });
...
program
  ...
  .action(async (forks, opts) => {
    ...
    if (opts.branch) input.branch = opts.branch;
    await run("forkctl_batch_sync", input, opts);
  });
...
program
  ...
  .option("--json", "raw JSON output")
  .action(async (operationId, opts) => {
    await run("forkctl_receipt", { operationId }, opts);
  });
...
program
  ...
  .action(async (opts) => {
    ...
    await run("forkctl_audit_log", input, opts);
  });
...
renameCmd
  .command("plan <path>")
  .description("Build a read-only rename plan and write .forkctl/rename-plan.{json,diff}")
  .requiredOption("--from <name>", "source canonical name (e.g. forkctl)")
...
renameCmd
  ...
  .action(async (repoPath, opts) => {
    ...
    await run("forkctl_rename_apply", input, opts);
  });
...
renameCmd
  ...
  .action(async (repoPath, opts) => {
    ...
    await run("forkctl_rename_rollback", input, opts);
  });
...
program.parseAsync(process.argv).catch((err) => {
  process.stderr.write(`forkctl cli fatal: ${err instanceof Error ? err.message : String(err)}\n`);
  process.exit(1);
});
