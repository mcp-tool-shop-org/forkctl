 ...
 * an operation_id that callers poll via forkable_check_operation.
 */
...
export class Operations {
  ...
  create(input: {
    ...
    if (!row) throw new ForkableError("INTERNAL", `Operation ${id} vanished after insert`);
    return row;
  }
  ...
  private requireGet(id: string): OperationRecord {
    ...
    if (!rec) throw new ForkableError("OPERATION_NOT_FOUND", `Operation ${id} not found`);
    return rec;
  }
}
