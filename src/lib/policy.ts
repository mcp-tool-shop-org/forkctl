 ...
 * Private repos are forkctl only if the org explicitly enables forking.
 *
...
export function policyBlocker(verdict: ForkPolicyVerdict): ForkctlError | null {
  if (verdict.allowed === "no") {
    return new ForkctlError("FORK_POLICY_BLOCKED", verdict.reason, {
      hint:
        verdict.source === "org_policy"
          ...
          : verdict.source === "repo_archived"
            ? "Unarchive the repo first, or use forkctl_create_from_template if it is a template."
