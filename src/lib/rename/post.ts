...
export async function runPostPass(opts: PostPassOptions): Promise<PostPassResult> {
  ...
  if (assetsToRegen.length > 0) {
    const manifestDir = path.join(opts.repoRoot, ".forkable");
    ...
    const manifest = {
      ...
      from: opts.from,
      to: opts.to,
      note: "These assets were NOT modified by `forkable rename`. Regenerate manually with your design pipeline.",
