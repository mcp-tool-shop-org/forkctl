 ...
 *
 * Snapshots live under `.forkctl/snapshots/rename-<unix-ts>/`.
...
export async function takeSnapshot(root: string): Promise<SnapshotResult> {
  ...
  const dir = path.join(root, ".forkctl", "snapshots", id);
  ...
  if (await isGit(root)) {
    ...
    // --include-untracked` would otherwise sweep the just-created `.forkctl/`
...
async function findLatestSnapshot(root: string): Promise<string | null> {
  const dir = path.join(root, ".forkctl", "snapshots");
...
export async function rollbackSnapshot(opts: RollbackOptions): Promise<RollbackResult> {
  ...
  if (!snapshotId) {
    throw new ForkctlError("RENAME_ROLLBACK_NOT_FOUND", "no rename snapshot to restore", {
      hint: "To fix: pass --snapshot-id or run `rename apply` first.",
    });
  }
  const snapDir = path.join(opts.repoRoot, ".forkctl", "snapshots", snapshotId);
  if (!(await exists(snapDir))) {
    throw new ForkctlError("RENAME_ROLLBACK_NOT_FOUND", `snapshot ${snapshotId} not found`, {
      hint: "To fix: check .forkctl/snapshots/ for available IDs.",
    });
  }
  ...
  if (await exists(preHeadPath)) {
    ...
    if (!r.ok) {
      throw new ForkctlError("RENAME_APPLY_FAILED", `git reset --hard failed: ${r.stderr.slice(0, 400)}`, {
  ...
  if (!(await exists(treeDir))) {
    throw new ForkctlError("RENAME_ROLLBACK_NOT_FOUND", "snapshot has no tree/ directory", {
