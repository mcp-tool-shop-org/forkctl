 ...
 *   4. Returns the operation_id; caller polls forkctl_check_operation
 *
...
export const createForkTool: ToolDescriptor<CreateForkInput, CreateForkOutput> = {
  name: "forkctl_create_fork",
  description:
    "Start an async fork of a source repo. Runs fork-policy preflight, kicks off the GitHub fork, and returns an operation_id. Poll forkctl_check_operation to track readiness (forks can take up to 5 minutes per GitHub docs).",
  ...
  handler: (input, ctx) =>
    safe(async () => {
      ...
      return {
        ...
        message:
          "Fork creation accepted. Poll forkctl_check_operation with the operation_id to track readiness.",
      };
    }),
};
...
async function getAuthenticatedLogin(octokit: import("@octokit/rest").Octokit): Promise<string> {
  try {
    ...
    const e = mapGitHubError(err);
    throw new ForkctlError(e.code, "Could not resolve authenticated user", {
