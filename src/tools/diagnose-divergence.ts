...
> = {
  name: "forkctl_diagnose_divergence",
  description:
    "Read-only report on how a fork's branch has diverged from its upstream. Returns status (ahead | behind | identical | diverged), aheadBy/behindBy counts, files at risk, and a fast-forward prediction. Call this BEFORE forkctl_sync to avoid surprises.",
...
function recommend(status: string, behindBy: number, aheadBy: number): string {
  switch (status) {
    ...
    case "behind":
      return `Fork is ${behindBy} commit(s) behind. Call forkctl_sync to fast-forward.`;
    case "ahead":
      return `Fork is ${aheadBy} commit(s) ahead. Sync is unnecessary; consider opening a PR upstream.`;
    case "diverged":
      return `Both have unique commits (ahead ${aheadBy}, behind ${behindBy}). Fast-forward not possible — call forkctl_propose_sync_pr to open an interactive resolution.`;
    default:
