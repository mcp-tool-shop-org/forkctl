...
export const receiptTool: ToolDescriptor<ReceiptInput, Receipt> = {
  name: "forkable_receipt",
  description:
    "Get a machine-readable receipt for any forkable operation: the operation record + every audit entry that referenced it, plus a one-line summary. Use this as the canonical proof of what forkable did.",
  inputSchema: ReceiptInputSchema,
  handler: (input, ctx) =>
    safe(async () => {
      const op = ctx.operations.get(input.operationId);
      if (!op) {
        throw new ForkableError("OPERATION_NOT_FOUND", `Operation ${input.operationId} not found`);
      }
