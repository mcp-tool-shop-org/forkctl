...
export const renameApplyTool: ToolDescriptor<RenameApplyInput, RenameApplyOutput> = {
  name: "forkable_rename_apply",
  description:
    "Apply a rename plan to the target repo. Takes a snapshot first (git stash or file tree), then executes layers A→B→C→D→E in order, and writes a receipt. Rollback available via forkable_rename_rollback.",
  inputSchema: RenameApplyInputSchema,
  handler: async (input) => {
    const logger = buildLogger({ defaultLevel: "info", baseFields: { tool: "forkable_rename_apply" } });
    const repoRoot = path.resolve(input.path);
    if (!(await exists(repoRoot))) {
      return fail(
        new ForkableError("RENAME_NOT_A_REPO", `path does not exist: ${repoRoot}`, {
    ...
    if (!(await exists(planAbs))) {
      return fail(
        new ForkableError("RENAME_PLAN_STALE", `plan file not found: ${planAbs}`, {
          hint: "To fix: run `forkable rename plan` first to produce a plan.json.",
        }),
      );
    }
    ...
    return safe(async (): Promise<RenameApplyOutput> => {
      ...
      try {
        ...
        if (err instanceof ForkableError) throw err;
        throw new ForkableError("RENAME_SNAPSHOT_FAILED", (err as Error).message, {
      ...
      const receiptDir = path.join(repoRoot, ".forkable");
