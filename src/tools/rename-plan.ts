...
export const renamePlanTool: ToolDescriptor<RenamePlanInput, RenamePlanOutput> = {
  name: "forkable_rename_plan",
  description:
    "Build a read-only rename plan: detect casing variants of `from`, enumerate every file that would change across identity/symbols/textual/post layers, and write a reviewable JSON + diff artifact under .forkable/.",
  inputSchema: RenamePlanInputSchema,
  handler: async (input) => {
    const logger = buildLogger({ defaultLevel: "info", baseFields: { tool: "forkable_rename_plan" } });
    ...
    if (input.from === input.to) {
      return fail(
        new ForkableError("RENAME_INVALID_NAME", "`from` and `to` must differ", {
