...
export const syncTool: ToolDescriptor<SyncInput, SyncOutput> = {
  name: "forkable_sync",
  description:
    "Sync a fork branch with its upstream parent via POST /repos/{owner}/{repo}/merge-upstream. Reports the merge type (fast-forward | merge | none) honestly. On conflict, surfaces SYNC_CONFLICT — call forkable_propose_sync_pr to open a PR-based sync.",
      ...
      try {
        ...
        if (e.code === "GITHUB_CONFLICT") {
          throw new ForkableError(
            ...
            {
              hint: "Call forkable_propose_sync_pr to open a PR-based sync that you can resolve interactively.",
