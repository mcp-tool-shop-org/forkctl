...
describe("forkctl_make_forkable", () => {
  it("plan mode produces patch steps for missing license/README/env", async () => {
    ...
    const result = await makeForkableTool.handler(
      { repo: "octocat/bare", mode: "plan", branch: "forkctl/adoption-fixes" },
      ctx(oct),
    );
  ...
  it("plan mode produces no steps for a healthy repo", async () => {
    ...
    const result = await makeForkableTool.handler(
      { repo: "octocat/healthy", mode: "plan", branch: "forkctl/adoption-fixes" },
      ctx(oct),
    );
    expect(result.ok).toBe(true);
    if (!result.ok) return;
...
describe("forkctl_make_forkable — pr mode (F-001)", () => {
  it("happy path: creates branch, commits files, opens PR, returns PR URL", async () => {
    ...
    const result = await makeForkableTool.handler(
      { repo: "octocat/bare", mode: "pr", branch: "forkctl/adoption-fixes" },
      ctx(oct),
    );
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    ...
    expect(ref.ref).toBe("refs/heads/forkctl/adoption-fixes");
    expect(ref.sha).toBe("deadbeef00");

    // File contents were committed on the new branch with forkctl-prefixed messages
    ...
    expect(firstFile.branch).toBe("forkctl/adoption-fixes");
    expect(firstFile.message.startsWith("forkctl:")).toBe(true);

    // PR created head→base on source repo
    const pr = calls.pullsCreate[0] as { head: string; base: string; title: string };
    expect(pr.head).toBe("forkctl/adoption-fixes");
    expect(pr.base).toBe("main");
    expect(pr.title).toMatch(/forkctl/);
  });
  ...
  it("pr mode with zero blockers returns without opening a PR", async () => {
    ...
    const result = await makeForkableTool.handler(
      { repo: "octocat/healthy", mode: "pr", branch: "forkctl/adoption-fixes" },
      ctx(oct),
    );
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it("pr mode: 422 on createRef (branch exists) — expected MAKE_FORKABLE_BRANCH_EXISTS once backend F-008 lands", async () => {
    ...
    //   code MAKE_FORKABLE_BRANCH_EXISTS (forkctl/errors.ts already declares
