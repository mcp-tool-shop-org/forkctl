...
describe("forkctl_propose_sync_pr", () => {
  it("creates a sync branch at upstream HEAD and opens a PR", async () => {
    ...
    const result = await proposeSyncPrTool.handler(
      {
        fork: "myhandle/fork",
        syncBranch: "forkctl/sync-from-upstream",
        prTitle: "forkctl: sync from upstream",
      },
      ctx(oct),
    );
    ...
    expect(ref.ref).toBe("refs/heads/forkctl/sync-from-upstream");
    const pr = calls.pullsCreate[0] as { head: string; base: string };
    expect(pr.head).toBe("forkctl/sync-from-upstream");
  ...
  it("stale sync branch at different SHA surfaces SYNC_BRANCH_EXISTS (backend F-003)", async () => {
    ...
    const oct = syncFakeOctokit({
      ...
      syncBranchName: "forkctl/sync-from-upstream",
    });
    const result = await proposeSyncPrTool.handler(
      {
        fork: "myhandle/fork",
