...
describe("forkable_receipt", () => {
  it("returns the operation + audit trail + summary", async () => {
    ...
    log.record({ tool: "forkable_create_fork", input: { source: "octocat/x" }, ok: true, result: { ok: true }, operationId: op.id });
...
describe("forkable_audit_log", () => {
  it("returns recent entries respecting filter", async () => {
    const c = ctx();
    const log = new AuditLog(c.db);
    log.record({ tool: "forkable_assess", input: { repo: "x/y" }, ok: true, result: { score: 80 } });
    log.record({ tool: "forkable_sync", input: { fork: "me/x" }, ok: false, result: null });

    const result = await auditLogTool.handler({ tool: "forkable_assess", limit: 50 }, c);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.data.count).toBe(1);
    expect(result.data.entries[0]!.tool).toBe("forkable_assess");
  });
});
