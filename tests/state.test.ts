...
describe("state path resolution", () => {
  it("honors FORKCTL_STATE_DIR", () => {
    expect(resolveStateDir({ FORKCTL_STATE_DIR: "/tmp/forkctl-x" })).toBe("/tmp/forkctl-x");
  });
  ...
  it("computes db path", () => {
    const path = resolveDbPath({ FORKCTL_STATE_DIR: "/tmp/x" });
    expect(path.replace(/\\/g, "/")).toBe("/tmp/x/forkctl-state.db");
  });
});
...
describe("openState", () => {
  ...
  it("is idempotent on repeat open of the same DB file (F-002)", () => {
    ...
    const tmpDir = mkdtempSync(join(tmpdir(), "forkctl-state-"));
